...
def daubcqf(N, TYPE="min"):
    ...
    h_0 = math.sqrt(2.0) * h_0 / torch.sum(h_0)
    ...
    assert (
        torch.abs(torch.sum(h_0 ** 2)) - 1 < 1e-4
    ), 'Numerically unstable for this value of "N".'
...
def _est_additive_noise(
    ...
    hld = (ddp + eps) @ torch.eye(
        int(dim0data), dtype=calculation_dtype, device=subdata.device
    )
    ddpi = torch.eye(
...
def vertical_difference(x: torch.Tensor, n: int = 1):
    ...
    ret = x
    for _ in range(n):
        # todo: torch.diff does the *last* axis but matlab diff
